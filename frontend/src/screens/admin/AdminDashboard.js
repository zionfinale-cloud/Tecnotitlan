...
const OperationalReconciliation = ({ data }) => {
  const inventoryLabels = { RESTOCKED: 'Reintegrado', PENDING_INSPECTION: 'Pendiente de inspección', PENDING_REVIEW: 'Pendiente de revisión' };
  return <div className={styles.reconciliation}><b>Seguimiento operativo</b><span>Inventario <strong>{inventoryLabels[data.inventoryStatus] || data.inventoryStatus || 'Pendiente'} ({data.restockedQuantity || 0}/{data.soldQuantity || 0})</strong></span><small>Los costos, comisiones, márgenes y conciliación monetaria requieren permiso financiero.</small></div>;
};
