...
const AdminDashboard = () => {
  ...
  const [quality, setQuality] = useState({}); const [alerts, setAlerts] = useState([]); const [days, setDays] = useState(30); const [error, setError] = useState(''); const [busyAlert, setBusyAlert] = useState('');
  const load = useCallback(() => Promise.all([api.get(`/analytics/dashboard?days=${days}`), api.get('/service-quality/dashboard'), api.get('/unified-inbox/critical-alerts')]).then(([views, service, critical]) => { setAnalytics(views.data.data); setQuality(service.data.data.summary || {}); setAlerts(critical.data.data.alerts || []); }).catch((err) => setError(err.response?.data?.message || 'No se pudieron cargar las métricas.')), [days]);
  ...
  return <div className={styles.dashboard}><header><div><span>Vista ejecutiva</span><h1>Dashboard de administración</h1><p>Tráfico, origen de visitantes y calidad de atención en una sola lectura.</p></div><select value={days} onChange={(e) => setDays(Number(e.target.value))}><option value="7">7 días</option><option value="30">30 días</option><option value="90">90 días</option></select></header>{error && <div className={styles.error}>{error}</div>}
    {alerts.length > 0 && <section className={styles.criticalAlerts} role="alert" aria-live="assertive"><div className={styles.criticalHeader}><i className="fas fa-exclamation-triangle" /><div><span>Atención inmediata</span><h2>{alerts.length} {alerts.length === 1 ? 'caso crítico requiere' : 'casos críticos requieren'} revisión</h2><p>Reclamos y reembolsos permanecen visibles hasta que un administrador o vendedor confirme que ya los revisó.</p></div></div><div className={styles.alertList}>{alerts.slice(0, 5).map((alert) => <article key={alert.id}><div><b>{alert.title}</b><p>{alert.message}</p><small>{alert.orderNumber || `Reclamo ${alert.claimId}`}</small></div><span><a href="/admin/inbox">Atender ahora</a><button disabled={busyAlert === alert.id} onClick={() => acknowledgeAlert(alert)}>{busyAlert === alert.id ? 'Guardando…' : 'Marcar revisado'}</button></span></article>)}</div>{alerts.length > 5 && <a className={styles.viewAll} href="/admin/inbox">Ver los {alerts.length} casos en Bandeja unificada</a>}</section>}
