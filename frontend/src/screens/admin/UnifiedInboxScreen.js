...
import resolutionStyles from './UnifiedInboxResolution.module.css';
...
const sourceLabels = { WHATSAPP: 'WhatsApp', SUPPORT: 'Ticket', MELI_QUESTION: 'Pregunta preventa', MELI_POST_SALE: 'Mensaje posventa', MELI_CLAIM: 'Reclamo', ORDER_CANCELLATION: 'Cancelación', TECATL: 'Tecatl' };
const slaLabels = { BREACHED: 'SLA vencido', AT_RISK: 'SLA por vencer', ON_TRACK: 'SLA en tiempo', MET: 'SLA atendido' };
const typeLabels = { CLAIM: 'Reclamos', CANCELLATION: 'Cancelaciones', RETURN: 'Devoluciones', QUESTION: 'Preguntas', PRIVATE_MESSAGE: 'Mensajes privados' };
const typeIcons = { CLAIM: 'fa-exclamation-triangle', CANCELLATION: 'fa-ban', RETURN: 'fa-undo-alt', QUESTION: 'fa-question-circle', PRIVATE_MESSAGE: 'fa-comment-dots' };

const UnifiedInboxScreen = () => {
  const [items, setItems] = useState([]);
  const [counts, setCounts] = useState({ total: 0, pending: 0, unlinked: 0, channels: {}, sections: {}, types: {} });
  ...
  const [section, setSection] = useState('IMPORTANT');
  ...
  }), [items, query, section, typeFilter, channel, onlyPending]);
  ...
  const availableTypes = section === 'IMPORTANT' ? ['CLAIM', 'CANCELLATION', 'RETURN'] : ['QUESTION', 'PRIVATE_MESSAGE'];
  ...
  return <div className={styles.page}>
    <header className={styles.header}><div><span>Atención 360°</span><h1>Bandeja unificada</h1><p>Casos críticos separados de las conversaciones comerciales, siempre ligados al pedido correcto.</p></div><button onClick={() => load()} disabled={busy}><i className="fas fa-sync-alt" /> Actualizar</button></header>
    {error && <div className={styles.error}>{error}</div>}{success && <div className={styles.success}>{success}</div>}
    <section className={styles.stats}><article><small>Total</small><strong>{counts.total || items.length}</strong></article><article className={styles.importantStat}><small>Casos importantes</small><strong>{counts.sections?.IMPORTANT || 0}</strong></article><article><small>Conversaciones</small><strong>{counts.sections?.CONVERSATIONS || 0}</strong></article><article><small>Requieren atención</small><strong>{counts.pending || 0}</strong></article></section>
    <nav className={styles.sections} aria-label="Secciones de la bandeja">
      <button className={section === 'IMPORTANT' ? styles.sectionActiveImportant : ''} onClick={() => { setSection('IMPORTANT'); setTypeFilter('ALL'); }}><i className="fas fa-shield-alt" /><span><b>Casos importantes</b><small>Reclamos, cancelaciones y devoluciones</small></span><em>{counts.sections?.IMPORTANT || 0}</em></button>
      <button className={section === 'CONVERSATIONS' ? styles.sectionActiveConversation : ''} onClick={() => { setSection('CONVERSATIONS'); setTypeFilter('ALL'); }}><i className="fas fa-comments" /><span><b>Conversaciones</b><small>Preguntas y mensajes privados</small></span><em>{counts.sections?.CONVERSATIONS || 0}</em></button>
    </nav>
    <div className={styles.typeFilters}><button className={typeFilter === 'ALL' ? styles.typeActive : ''} onClick={() => setTypeFilter('ALL')}>Todos</button>{availableTypes.map((value) => <button key={value} className={typeFilter === value ? styles.typeActive : ''} onClick={() => setTypeFilter(value)}><i className={`fas ${typeIcons[value]}`} /> {typeLabels[value]} <span>{counts.types?.[value] || 0}</span></button>)}</div>
    ...
    {items.length === 0 ? <div className={styles.empty}><i className="fas fa-check-circle" /><h2>Todo está tranquilo</h2><p>Los nuevos contactos de todos los canales aparecerán aquí.</p></div> : <div className={styles.workspace}>
      <aside className={styles.list}>{filtered.length ? filtered.map((item) => <button key={item.id} className={`${item.id === selected?.id ? styles.active : ''} ${item.important ? styles.importantItem : ''}`} onClick={() => { setSelectedId(item.id); setReply(''); setShowOrderSearch(false); }}><span><b><i className={`fas ${channelIcons[item.channel] || 'fa-comment'}`} /> {item.channel}</b>{item.unreadCount > 0 && <em>{item.unreadCount}</em>}</span><small className={`${styles.typeBadge} ${item.important ? styles.importantBadge : ''}`}><i className={`fas ${typeIcons[item.type] || 'fa-comment'}`} /> {item.typeLabel}</small><strong>{item.title}</strong><p>{item.preview || 'Sin vista previa'}</p><small>{sourceLabels[item.sourceType]} · {fmtDate(item.timestamp)}</small>{item.sla && <small className={styles.orderTag}>{slaLabels[item.sla.state]}{item.sla.remainingMinutes != null ? ` · ${Math.abs(item.sla.remainingMinutes)} min` : ''}</small>}{item.linkedOrder && <small className={styles.orderTag}><i className="fas fa-link" /> {item.linkedOrder.orderNumber}</small>}</button>) : <div className={styles.noResults}>No hay elementos en esta sección con los filtros seleccionados.</div>}</aside>
      {selected && <main className={styles.detail}>
        <section className={styles.conversation}><div className={styles.title}><div><span>{selected.channel} · {sourceLabels[selected.sourceType]}</span><h2>{selected.title}</h2><p>{selected.customer?.name || selected.customer?.email || selected.customer?.phone || `Referencia ${selected.sourceId}`}</p>{selected.outcome && <b className={resolutionStyles.outcome}><i className="fas fa-check-circle" /> {selected.outcome.headline}</b>}{selected.sla && <b>{slaLabels[selected.sla.state]} · objetivo {selected.sla.targetMinutes} min{selected.sla.dueAt ? ` · vence ${fmtDate(selected.sla.dueAt)}` : ''}</b>}</div><a href={selected.deepLink}>Abrir módulo especializado <i className="fas fa-external-link-alt" /></a></div><div className={styles.messages}>{selected.messages?.length ? selected.messages.map((message) => <article key={message.id} className={message.direction === 'SYSTEM' ? resolutionStyles.system : message.direction === 'OUTBOUND' ? styles.outbound : styles.inbound}><b>{message.direction === 'SYSTEM' ? 'Mercado Libre' : message.direction === 'OUTBOUND' ? 'Tecnotitlan' : 'Cliente'}</b><p>{message.text || 'Mensaje sin texto visible'}</p><small>{fmtDate(message.at)}{message.status ? ` · ${message.status}` : ''}</small></article>) : <p>Este expediente no contiene mensajes visibles.</p>}</div>{selected.canReply ? <form onSubmit={submitReply}><select defaultValue="" onChange={(event) => { applyTemplate(event.target.value); event.target.value = ''; }}><option value="">Insertar plantilla segura...</option>{templates.filter((entry) => !entry.sourceType || entry.sourceType === selected.sourceType).map((entry) => <option key={entry.id} value={entry.id}>{entry.category} · {entry.name}</option>)}</select><textarea rows="4" maxLength={selected.maxLength || 5000} value={reply} onChange={(event) => setReply(event.target.value)} placeholder="Escribe una respuesta clara y segura..." /><div><small>{reply.length}/{selected.maxLength || 5000}</small><button disabled={busy || !reply.trim()}>Enviar respuesta</button></div></form> : <div className={styles.locked}><i className="fas fa-lock" /> Este canal no admite respuesta en su estado actual.</div>}</section>
...
const OrderPanel = ({ item, busy, showSearch, setShowSearch, orderQuery, setOrderQuery, results, searchOrders, linkOrder, unlinkOrder }) => {
  ...
  return <section className={styles.orderPanel}><div className={styles.orderHeader}><div><span>Contexto comercial</span><h3>{order ? order.orderNumber : 'Sin pedido ligado'}</h3></div>{item.linkable !== false && <button onClick={() => setShowSearch(!showSearch)}>{order ? 'Cambiar pedido' : 'Buscar pedido'}</button>}</div>
